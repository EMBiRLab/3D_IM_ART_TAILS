...
for i = 1 : length(PRY_traj_set)
    ...
    [A, b] = linear_ineq_con(nsteps, nstates, ninputs, dt, inputrate_range, inputacc_range);
    [Aeq, beq] = linear_eq_con(nsteps, nstates, ninputs, x0);
    [lb, ub] = bound_con(nsteps, state_range, input_range);

    nonlcon = @(z) nonlinear_con(z, nsteps, nstates, ninputs, torso_dof, dt, power_limit, state_range, urdf_filename_drd, urdf_filepath_pino, opt_coeff.con);
    ...
    if saveon
        ...
        save([savedata_dir, filesep, 'traj_opt_saved_data.mat'], ...
             ...
             'PRY_traj', 'PRY_ref_name_widx', 'options', 'fval', 'zsol', 'X', 'U');
