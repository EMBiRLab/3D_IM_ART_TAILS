...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs != 5) {
        mexErrMsgTxt("Expect 5 inputs: urdf_filename, q, qd, u, link_lengths!\n");
    }
    ...
    size_t link_lengths_m = mxGetM(prhs[4]);
    size_t link_lengths_n = mxGetN(prhs[4]);
    ...
    if (link_lengths_m * 2 != model.nv - 3) {
        mexErrMsgTxt("Expect size of link_lengths to be correct! This is specifically designed for tailed robot project!\n");
    }

    if (link_lengths_n != 1) {
        mexErrMsgTxt("Expect link_lengths to be a column vector!\n");
    }
    ...
    Eigen::VectorXd link_lengths = Eigen::VectorXd::Zero(link_lengths_m);
    copyPointerToEigenVector(link_lengths, (double*)mxGetData(prhs[4]), link_lengths_m);
    ...
    // Modify the robot inertial parameters according to the link lengths
    const double rho = 2;
    for (size_t i = 0; i < link_lengths_m; i++) {
        double mass = rho * link_lengths(i);
        model.inertias[3 + 2 * (i + 1)].mass() = mass;
        model.inertias[3 + 2 * (i + 1)].lever()(1) = 0.5 * link_lengths(i);
        double box_dim[3] = {0.1, link_lengths(i), 0.1};
        model.inertias[3 + 2 * (i + 1)].inertia().setZero();
        model.inertias[3 + 2 * (i + 1)].inertia().data()(0) = mass*(box_dim[1]*box_dim[1] + box_dim[2]*box_dim[2])/12;
        model.inertias[3 + 2 * (i + 1)].inertia().data()(2) = mass*(box_dim[0]*box_dim[0] + box_dim[2]*box_dim[2])/12;
        model.inertias[3 + 2 * (i + 1)].inertia().data()(5) = mass*(box_dim[0]*box_dim[0] + box_dim[1]*box_dim[1])/12;
    }

    for (size_t i = 1; i < link_lengths_m; i++) {
        model.jointPlacements[3 + 2 * (i + 1) - 1].translation()(1) = link_lengths(i);
    }
