function [name, links, joints, transmissions] = ros_load_urdf(urdf_file)
    ...
    for i=0:num_joints-1
        ...
        if xml_joint.hasChildNodes
            ...
            joints(index).friction = 0.;
            joints(index).damping = 0.;
            xml_dynamics = xml_joint.getElementsByTagName('dynamics').item(0);
            if ~isempty(xml_dynamics)
                friction = xml_dynamics.getAttribute('friction');
                damping = xml_dynamics.getAttribute('damping');
                if ~isempty(char(friction))
                    joints(index).friction = str2num(friction);
                end
                if ~isempty(char(damping))
                    joints(index).damping = str2num(damping);
                end
            end
            index = index + 1;
        end
